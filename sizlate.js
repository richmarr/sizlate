...
exports.__express = function(filename, options, callback) {
	...
	var wait = false;
	var extention = options.settings['view engine'];
	...
	for(var key in selectors) {
		if(selectors[key] && selectors[key].partial){// this is a partial.
			if(selectors[key].data && selectors[key].data.length > 0){ // make sure we are passed in data and that the data is not empty.
				wait = true;
				count++;
				fs.readFile(options.settings.views + '/partials/' + selectors[key].partial + '.'+extention, 'utf8', function ( key, err, data) {
					if(err) {
	...
	var doRendering = function() {
		if(options.layout) {
			fs.readFile(options.settings.views + '/' + options.layout + '.'+ extention, 'utf8', function(error, template) {
